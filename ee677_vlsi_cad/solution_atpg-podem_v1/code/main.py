from netlist import read_text_netlist, netlist_to_nodes
from gates import update_row, update_row_imply
...
def backtrace(nG, nV):
    for i in range (0,len(netlist)): 
        if ( int(netlist[i][2]) == nG):
            ...
            row_v = update_row(nV,row_values)
...
def imply_device(device_id):
    ...
    row_update = update_row_imply(row_imply)
...
def main_podem(nG_C, nV):
    global netlist, ins, outs, node_map, node_values
    netlist = read_text_netlist("netlist.txt")
    num_nodes, ins, outs, node_map = netlist_to_nodes(netlist)
    create_devicemap()
    node_values = [-5] * (num_nodes + 1)
    print(node_values)
